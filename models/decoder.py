...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, in_feats, num_tokens, block, layers, zero_init_residual=False, width_per_group=64):
        super().__init__()
        norm_layer = nn.BatchNorm1d
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, in_feats, embed_dim, block, layers, zero_init_residual=False, width_per_group=64):
        super().__init__()
        norm_layer = nn.BatchNorm1d
        self._norm_layer = norm_layer
        self.inplanes = 128
        ...
        self.fc = nn.Conv1d(384 * block.expansion, embed_dim, 1)
...
def resnet18_decoder(in_feats, num_tokens):
    return ResNetDecoder(in_feats, num_tokens, BasicBlock, [2, 2, 2, 2], zero_init_residual=True)


def resnet34_decoder(in_feats, num_tokens):
    return ResNetDecoder(in_feats, num_tokens, BasicBlock, [3, 4, 6, 3], zero_init_residual=True)


def resnet18_encoder(in_feats, embed_dim):
    return ResNetEncoder(in_feats, embed_dim, BasicBlock, [2, 2, 2, 2], zero_init_residual=True)

    
def resnet34_encoder(in_feats, embed_dim):
    return ResNetEncoder(in_feats, embed_dim, BasicBlock, [3, 4, 6, 3], zero_init_residual=True)
