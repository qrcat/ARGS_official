...
class CosineWarmupScheduler(optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, num_warmup_steps: int, num_training_steps: int, fix_first_lr: float=1e-6):
    ...
    def get_lr_factor(self, epoch):
        ...
        elif epoch < self.max_num_epoch:
            lr_factor = 0.5 * (1 + np.cos(np.pi * (epoch - self.warmup) / (self.max_num_epoch-self.warmup)))
        else:
            lr_factor = 0
